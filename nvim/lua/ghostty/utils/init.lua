...
function utils.load(...)
  ...
  local options = require("ghostty.config").options
...
function utils.darken(hex, amount, bg)
  ...
  return require("ghostty.utils.colorblind").daltonize(darken, options.colorblind.severity)
end
...
function utils.make_global_bg(transparent)
  ...
  if options.colorblind.enable and options.colorblind.preserve_background then
    return require("ghostty.colors").default_bg
  end
